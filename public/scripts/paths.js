...
window.addEventListener('parampara:langchange', () => {
    ...
    if (modal && modal.classList.contains('active') && currentPath) {
        document.getElementById('path-player-title').textContent = pathTitle(currentPath);
        displayPathStep();
    }
});
    ...
    return [
        {
            ...
            description: 'The basic running stitch used in Kantha embroidery, layering old saris to create warmth and beauty.',
            imageUrl:    '',
            audioUrl:    ''
        },
        {
            ...
            description: 'Intricate motifs depicting fish, lotus flowers, and village life stitched in bright threads.',
            imageUrl:    '',
            audioUrl:    ''
        },
        {
            ...
            description: 'A village elder recounts how Kantha began as a way to recycle worn saris into quilts for newborns.',
            imageUrl:    '',
            audioUrl:    ''
        },
        {
            ...
            description: 'Fish are considered auspicious in Mithila culture and appear in almost every Madhubani painting.',
            imageUrl:    '',
            audioUrl:    ''
        },
        {
            ...
            description: 'Traditional paintings done on the walls of the bridal chamber, depicting bamboo groves and lotus ponds.',
            imageUrl:    '',
            audioUrl:    ''
        }
    ];
...
function pathTitle(path) { return path.titleKey ? tPath(path.titleKey) : (path.title       || ''); }
function pathTheme(path) { return path.themeKey ? tPath(path.themeKey) : (path.theme       || ''); }
function pathDesc(path)  { return path.descKey  ? tPath(path.descKey)  : (path.description || ''); }
...
function displayPathStep() {
    ...
    document.getElementById('next-step').disabled = currentStepIndex === items.length - 1;
}
